...
class AnalyzDashboard:
    ...
    def create_modern_header(self):
        ...
        subtitle_label = tk.Label(
            content_frame,
            text=f"Toplam {len(self.df)} ürün detaylı analizi",
            font=('Segoe UI', 14),
            fg='#bfdbfe',
            bg=self.colors['primary']
        )
    ...
    def create_enhanced_kpi_section(self):
        ...
        DashboardComponents.create_modern_kpi_card(kpi_row1, "💰", "Toplam Net Kar", f"₺{kpi_data.get('toplam_kar', 0):,.0f}", self.colors['success'], 0)
        DashboardComponents.create_modern_kpi_card(kpi_row1, "🏆", "En Karlı Ürün", en_karli_urun_text, self.colors['primary'], 1)
        DashboardComponents.create_modern_kpi_card(kpi_row1, "📈", "Ortalama Kar", f"₺{kpi_data.get('ortalama_kar', 0):,.0f}", self.colors['warning'], 2)
        DashboardComponents.create_modern_kpi_card(kpi_row1, "📦", "Toplam Ürün", f"{kpi_data.get('toplam_urun', 0)} adet", self.colors['info'], 3)
        
        # İkinci satır KPI kartları
        kpi_row2 = tk.Frame(kpi_container, bg=self.colors['bg_primary'])
        kpi_row2.pack(fill='x')
        
        DashboardComponents.create_modern_kpi_card(kpi_row2, "✅", "Karlı Ürün", f"{kpi_data.get('pozitif_kar_urun', 0)} adet", self.colors['success'], 0)
        DashboardComponents.create_modern_kpi_card(kpi_row2, "❌", "Zararlı Ürün", f"{kpi_data.get('negatif_kar_urun', 0)} adet", self.colors['danger'], 1)
        DashboardComponents.create_modern_kpi_card(kpi_row2, "🎯", "En Yüksek Kar", f"₺{kpi_data.get('en_karli_urun_kar', 0):,.0f}", '#8b5cf6', 2)
        DashboardComponents.create_modern_kpi_card(kpi_row2, "📊", "Toplam Satış", f"{kpi_data.get('toplam_satis_miktar', 0):,.0f} adet", self.colors['info'], 3)
    ...
    def create_performance_tab(self):
        ...
        DashboardComponents.create_modern_product_list(left_frame, top_profitable, 'Net Kar', self.colors['success'], self.analiz)
        DashboardComponents.create_modern_product_list(right_frame, top_selling, miktar_col if miktar_col else 'Miktar', self.colors['info'], self.analiz)
    ...
    def create_profit_tab(self):
        ...
        DashboardComponents.create_modern_product_list(low_perf_frame, low_profit_products, 'Net Kar', self.colors['danger'], self.analiz)
    ...
    def create_enhanced_search_section(self):
        ...
        for text, filter_type, color in filters:
            ...
            # Hover efekti
            if color == self.colors['text_secondary']:
                hover_color = '#4b5563'
    ...
    def apply_quick_filter(self, filter_type):
            ...
            if filter_type == "all":
                ...
                if self.analiz:
                    ...
                    if miktar_col and miktar_col in self.df.columns:
                        try:
                            threshold = self.df[miktar_col].quantile(0.75)
                            results = self.df[self.df[miktar_col] >= threshold]
                        except (ValueError, TypeError):
                            results = pd.DataFrame()
                    else:
                        results = pd.DataFrame()
        ...
        tk.Label(
            result_header,
            text=f"🎯 {filter_names[filter_type]}: {len(results)} sonuç",
            font=('Segoe UI', 14, 'bold'),
            fg=self.colors['info'],
            bg=self.colors['bg_secondary']
        ).pack(anchor='w')
